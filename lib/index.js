...
function thetaFunction(grafo) {
  let suma1 = sumEdgesWeight(grafo);
  visitedNodesFalse(grafo);
  dijkstra(grafo, grafo.nodes[0]);
  let nds = grafo.nodes;
  let suma2 = 0.0;
...
function copyGraph(grafo) {
  ...
  for (let i = 0; i < nodes.length; i++) {
    g2.addNode(new Node(nodes[i].x, nodes[i].y));
  }
  for (let i = 0; i < g2.nodes.length; i++) {
    let edt = grafo.nodes[i].edges;
    for (let j = 0; j < edt.length; j++) {
      let nod1 = new Node(edt[j].origin.x, edt[j].origin.y);
      let nod2 = new Node(edt[j].destiny.x, edt[j].destiny.y);
      g2.addEdge(nod1, nod2);
    }
  }
  visitedNodesFalse(grafo);
  visitedEdgesFalse(grafo);
  visitedNodesFalse(g2);
  visitedEdgesFalse(g2);
  return g2;
}
...
function isConected(grafo) {
  visitedEdgesFalse(grafo);
  visitedNodesFalse(grafo);
  let g = copyGraph(grafo);
  checkConectivity(g, g.nodes[0], g.nodes[0]);
  let nds = g.nodes;
  for (let i = 0; i < nds.length; i++) {
    if (!nds[i].visited) {
...
function proposeGraph(grafo) {
  ...
  while (stay) {
    ...
    while (ran1 === ran2) {
      ran2 = Math.floor(getRandom(0, sizeG));
    }
...
function probTransition(grafo1, grafo2, temp) {
  let theta1 = thetaFunction(grafo1);
  let theta2 = thetaFunction(grafo2);
...
for (let i = 0; i < 30; i++) {
  // Console.log(Math.floor(getRandom(0, 5)));
}
// Let boole = false;
// let g2 = copyGraph(g);
